...
```text
默认混合：沪深、ETF、A 股指数使用腾讯；北交所、韩国 KOSPI 使用东方财富
仅腾讯：全部请求腾讯，不支持的证券显示为空且暂停对应提醒判断
仅东财：全部请求东方财富，接口受限时允许显示暂不可用
```

该选择只影响实时行情、提醒使用的报价和分时趋势。筹码、成交额、资金流等研究与统计模块使用各自独立的数据源。切换模式后的第一批行情只用于校准提醒基准。
     ...
     expiresAt 未到期          无记录或已过期
       ...
       返回 HIT       获取证券级更新锁      获取证券级更新锁
              ...
              MISS / REFRESHED      有旧值返回 STALE
                  ...
                  浏览器直连恢复成功                    浏览器直连失败
                         │                                   │
              POST recover 校验并更新                    使用 STALE 旧值
...
- 返回 `cacheStatus:STALE` 或 `stale:true`；
- HTTP 502 或 504；
- 网络异常或请求超时；
- 返回结构为 `success:false`；
- 返回 HTTP 200，但缺少有效 `data`。

正式 GET 与浏览器直连均设置 10 秒前端超时；Cloudflare Worker 服务端请求超时为 8 秒。收到 `STALE` 时，前端先尝试浏览器直连并回传；直连或恢复上报失败才使用服务端返回的旧值。浏览器恢复链路不是独立数据源，也不绕过服务端缓存规则，成功数据必须回传并通过校验后才能参与筹码计算和共享。
...
- 上游失败且存在旧值时返回 `STALE`，前端直连恢复成功则使用并上报新数据，直连或恢复上报失败才使用旧值，筹码图仍能打开；
