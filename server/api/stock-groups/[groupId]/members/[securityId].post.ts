...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (error && typeof error === 'object' && 'issues' in error) return apiFailure(event, new ApiResponseError(422, 'INVALID_PAYLOAD', '转移参数不合法', error))
    return apiFailure(event, new ApiResponseError(500, 'STORAGE_WRITE_FAILED', '转移证券失败'))
  }
})
